...
object ReverbServer {
    ...
    private val json = Json {
        ...
        encodeDefaults = true  // type 같은 기본값 필드도 직렬화 포함
    }
        ...
        engine = embeddedServer(CIO, port = PORT) {
            ...
            routing {
                ...
                post("/api/test-notification") {
                    android.util.Log.i("ReverbServer", "=== 웹에서 테스트 알림 요청 받음 ===")
                    android.util.Log.i("ReverbServer", "현재 WebSocket 세션 수: ${WebSocketManager.sessionCount}")
                    
                    val testPayload = NotificationPayload(
                        id = "web_" + UUID.randomUUID().toString(),
                        timestamp = System.currentTimeMillis(),
                        packageName = "com.reverb.web.test",
                        appLabel = "웹 테스트",
                        category = "generic",
                        title = "🌐 웹에서 보낸 테스트 알림",
                        ...
                        priority = 0,
                        sbnKey = "test_web_${System.currentTimeMillis()}"
                    )

                    try {
                        NotificationStore.push(testPayload)
                        android.util.Log.i("ReverbServer", "NotificationStore에 저장 완료")
                        
                        broadcastNotification(testPayload)
                        android.util.Log.i("ReverbServer", "broadcastNotification 호출 완료")

                        call.respond(mapOf("success" to true, "message" to "테스트 알림 전송 완료"))
                    } catch (e: Exception) {
                        android.util.Log.e("ReverbServer", "테스트 알림 전송 중 오류", e)
                        call.respond(
                            HttpStatusCode.InternalServerError,
